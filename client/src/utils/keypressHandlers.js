// Handlers for the APL input codes a server may ⎕NQ at an object, keyed by the
// code Dyalog reports for that keystroke (see EWC/processEvent.aplf, which maps
// browser key names to APLKeys before the callback sees them).
//
// These only run for programmatic ⎕NQ — a real keystroke is handled natively by
// the browser, so nothing here is on the typing path. Printable characters are
// inserted by src/utils/insertTextIntoInput.js instead.
//
