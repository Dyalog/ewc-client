...
test.describe('DemoGrid - Phase 7 Component Embedding Debug', () => {
  let browser: Browser;
  let page: Page;
  ...
  test('debug: check if Input and CellTypes are received', async () => {
    ...
    const logs: string[] = [];
    page.on('console', msg => {
      if (msg.text().includes('Grid Debug')) {
        logs.push(msg.text());
      }
    });
    ...
    const cellsWithComponents = page.locator('.grid-cell.has-component');
    ...
    const headers = page.locator('.grid-col-header');
  ...
  test('debug: check cell content types', async () => {
    ...
    const cellsWithInputs = page.locator('.grid-cell input');
    ...
    const cellsWithButtons = page.locator('.grid-cell button');
    ...
    const checkboxes = page.locator('.grid-cell input[type="checkbox"]');
  ...
  test('debug: evaluate Grid state from React', async () => {
