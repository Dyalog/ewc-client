...
test.describe('Grid long-cell focused screenshots', () => {
  let browser: Browser;
  let page: Page;
  ...
  test('A: click cell (3,1) — different row, still wide cell', async () => {
    const grid = page.locator('.grid').nth(0);
    const cell = grid.locator('td.grid-cell[data-row="3"][data-col="1"]');
    await cell.click();
    await page.waitForTimeout(400);
  ...
  test('C: click cell (1,2) - 350px ultra-wide cell with long text', async () => {
    const grid = page.locator('.grid').nth(2);
    const cell = grid.locator('td.grid-cell[data-row="1"][data-col="2"]');
    await cell.click();
    await page.waitForTimeout(400);
  ...
  test('A: click after scrolling horizontally (cell partially clipped)', async () => {
    const grid = page.locator('.grid').nth(0);
    const container = grid.locator('.grid-container');
