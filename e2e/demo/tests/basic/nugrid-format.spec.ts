...
test.describe('DemoNuGridFormat - FormatString support', () => {
  ...
  test('FormatCell response after edit normalizes APL high minus', async () => {
    ...
    const cell = page.locator('.nugrid-cell[data-row="3"][data-col="5"]');
  ...
  test('typed high minus stays visible while editing, normalizes on blur', async () => {
    ...
    const cell = page.locator('.nugrid-cell[data-row="7"][data-col="5"]');

    // Select the cell so its input becomes editable (matches the pattern
    // used in nugrid.spec.ts "Edit cell allows typing without reverting").
    await cell.click();
    await new Promise(r => setTimeout(r, 200));
  ...
  // While editing a NuGrid Edit cell, ArrowLeft/ArrowRight must move the
  ...
  test('arrow keys move text cursor inside Edit, not the grid cell', async () => {
    ...
    const cell = page.locator('.nugrid-cell[data-row="2"][data-col="1"]');
