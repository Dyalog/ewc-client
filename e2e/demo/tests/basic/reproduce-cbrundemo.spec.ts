...
test('navigate menu rapidly and many times', async () => {
  ...
  // First navigation - load Grid
  page = await navigateToDemo(page, 'Grid', '.grid-table', 15000);
  console.log('-- first demo (Grid) loaded');
  ...
  const sequence = ['GridLongCell', 'Grid', 'GridLongCell', 'GridFormat', 'GridStyles', 'Grid'];
...
test('click menu, then immediately click an inner cell', async () => {
  ...
  const cell = page.locator('td.grid-cell').first();
  ...
  await page.getByRole('option', { name: 'GridLongCell', exact: true }).click({ timeout: 3000 }).catch(() => {});
