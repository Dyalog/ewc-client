...
//   "When clicking on an editable cell with long content (in a Grid
...
// full cell width.
test.describe('Grid long-cell selection border', () => {
  let browser: Browser;
  let page: Page;
  ...
  test('Grid A: clicking a long-content cell selects it visibly', async () => {
    ...
    // Use Grid A (the first Grid). Click the cell at row 1, col 1
    // (long product description, ~300px wide cell).
    const gridA = grids.nth(0);
    const cell = gridA.locator('td.grid-cell[data-row="1"][data-col="1"]');
    ...
    await page.screenshot({
      path: 'test-results/grid-longcell-A-row1col1.png',
      fullPage: false,
    });
    ...
    const measurements = await cell.evaluate((el) => {
      ...
      const parent = el.closest('.grid-container') as HTMLElement | null;
  ...
  test('Grid A: scroll right then click — border on partially-visible cell', async () => {
    const grids = page.locator('.grid');
    const gridA = grids.nth(0);
    const container = gridA.locator('.grid-container');
    ...
    await page.screenshot({
      path: 'test-results/grid-longcell-A-row1col1-scrolled.png',
      fullPage: false,
    });
  ...
  test('Grid A: click an Edit cell after scrolling rows', async () => {
    ...
    const visibleCell = gridA.locator('td.grid-cell[data-col="1"]').first();
    await visibleCell.click();
    await page.waitForTimeout(300);

    await page.screenshot({
      path: 'test-results/grid-longcell-A-vscrolled.png',
      fullPage: false,
    });
  ...
  test('Grid C: very wide column with long content', async () => {
    ...
    const cell = gridC.locator('td.grid-cell[data-row="1"][data-col="2"]'); // 350px col
    await expect(cell).toBeVisible();
    await cell.click();
    await page.waitForTimeout(300);
    await expect(cell).toHaveClass(/\bselected\b/);
    ...
    await page.screenshot({
      path: 'test-results/grid-longcell-C-wide.png',
      fullPage: false,
    });
  ...
  test('keyboard navigation scrolls the focused cell into view (Grid A)', async () => {
    const gridA = page.locator('.grid').nth(0);
    const container = gridA.locator('.grid-container');

    await gridA.locator('.grid-cell[data-row="1"][data-col="1"]').click();
    ...
    const right = await container.evaluate((el: HTMLElement) => {
      const sel = el.querySelector('.grid-cell.selected') as HTMLElement | null;
  ...
  test('edge navigation: ArrowRight at the end of an edit moves to the next cell', async () => {
    const gridA = page.locator('.grid').nth(0);
    const selectedCol = () => gridA.locator('.grid-cell.selected').getAttribute('data-col');

    await gridA.locator('.grid-cell[data-row="2"][data-col="1"]').click();
