...
// bounding-box gaps before/after dragging the splitter. Based on DemoSplitters
...
test.describe('DemoSplitters — Attach', () => {
  ...
  // which widens the left pane). Uses mouse so the splitter's
  ...
  test('dragging the splitter resizes the left pane (precondition)', async () => {
    ...
    // The pane must actually change width
  ...
  test('dynamic AutoConf via eWS toggles attach on/off (server round-trip)', async () => {
    ...
    // eWS AutoConf 0 -> attach disabled -> fixed width on resize.
    await toggle.click();
    ...
    // eWS AutoConf 3 -> attach re-enabled -> stretches again.
    await toggle.click();
    await page.waitForTimeout(500);
