...
// Grid TitleWidth / TitleHeight follow ⎕WC semantics:
//   unset / negative / empty → auto-size to fit content
//   0                         → hidden
//   positive                  → fixed pixels
test.describe('DemoGridTitles - Title auto-sizing', () => {
  let browser: Browser;
  let page: Page;
  ...
  const cornerWidth = async (gridId: string) => {
    const corner = page.locator(`#${gridId.replace(/\./g, '\\.')} .grid-corner-cell`);
    return await corner.evaluate((el) => el.getBoundingClientRect().width);
  };
  const headerRowHeight = async (gridId: string) => {
    const row = page.locator(`#${gridId.replace(/\./g, '\\.')} .grid-header-row`);
  ...
  test('shortRows render as three separate row titles, not one', async () => {
    const headers = page.locator('#F1\\.G1 .grid-row-header');
  ...
  test('fixed TitleWidth: explicit pixel value is honored exactly', async () => {
    ...
    const rowHeader = page.locator('#F1\\.G3 .grid-row-header').first();
  ...
  test('col titles are not truncated to an ellipsis', async () => {
    const headers = page.locator('#F1\\.G1 .grid-col-header');
  ...
  test('G5: long col title drives column 1 wider than columns 2/3', async () => {
    const headers = page.locator('#F1\\.G5 .grid-col-header');
  ...
  test('G5: long cell content overflows with ellipsis (cells are not measured)', async () => {
    ...
    const cell = page.locator('#F1\\.G5 .grid-cell[data-row="1"][data-col="2"]');
  ...
  test('G5: grid container is vertically scrollable (more rows than fit)', async () => {
    const container = page.locator('#F1\\.G5 .grid-container');
  ...
  test('G5: long cell content in scrolled-out row exists in DOM', async () => {
    ...
    const cell = page.locator('#F1\\.G5 .grid-cell[data-row="8"][data-col="3"]');
  ...
  test('G5: single-char cells render one character per cell, not concatenated', async () => {
    const row3col1 = page.locator('#F1\\.G5 .grid-cell[data-row="3"][data-col="1"]');
    const row3col2 = page.locator('#F1\\.G5 .grid-cell[data-row="3"][data-col="2"]');
    const row3col3 = page.locator('#F1\\.G5 .grid-cell[data-row="3"][data-col="3"]');
  ...
  test('G6: TitleHeight 0 unmounts the col-title row (no thead)', async () => {
    ...
    const colHeaders = page.locator('#F1\\.G6 .grid-col-header');
  ...
  test('G7: TitleWidth 0 unmounts the row-title column (no row headers)', async () => {
    ...
    // AND the corner cell.
    const rowHeaders = page.locator('#F1\\.G7 .grid-row-header');
    await expect(rowHeaders).toHaveCount(0);
    const corner = page.locator('#F1\\.G7 .grid-corner-cell');
  ...
  test('G8: omitting ColTitles defaults to A, B', async () => {
    const colHeaders = page.locator('#F1\\.G8 .grid-col-header');
  ...
  test('G9: omitting RowTitles defaults to 1, 2, 3', async () => {
    const rowHeaders = page.locator('#F1\\.G9 .grid-row-header');
  ...
  test('G10: Values only — both bands get Excel-style defaults', async () => {
    ...
    // Data cells render.
    await expect(page.locator('#F1\\.G10 .grid-cell')).toHaveCount(6); // 3×2
    await expect(
      page.locator('#F1\\.G10 .grid-cell[data-row="1"][data-col="1"]'),
    ).toHaveText('Alpha');
    // Col headers: A, B
    const colHeaders = page.locator('#F1\\.G10 .grid-col-header');
    await expect(colHeaders).toHaveCount(2);
    await expect(colHeaders.nth(0)).toHaveText('A');
    await expect(colHeaders.nth(1)).toHaveText('B');
    // Row headers: 1, 2, 3
    const rowHeaders = page.locator('#F1\\.G10 .grid-row-header');
    await expect(rowHeaders).toHaveCount(3);
    await expect(rowHeaders.nth(0)).toHaveText('1');
    // Corner cell present (since both bands exist).
    await expect(page.locator('#F1\\.G10 .grid-corner-cell')).toHaveCount(1);
  });
  ...
  test('auto CellWidths also never shrink below the FALLBACK floor (100px)', async () => {
    ...
    const colA = page.locator('#F1\\.G1 .grid-col-header').nth(0);
