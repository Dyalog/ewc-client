...
test.describe('DemoGridFlex - Flex Row', () => {
  let browser: Browser;
  let page: Page;
  ...
  test.beforeAll(async () => {
    ...
    page = await navigateToDemo(result.page, 'GridFlex', '.grid-table', 10000);
  });
  ...
  test('row flex grids render correct data', async () => {
    const g1 = page.locator('#F1\\.ROW\\.G1');
    const colHeaders = g1.locator('.grid-col-header');
    ...
    const cells = g1.locator('.grid-cell');
    await expect(cells).toHaveCount(9);
  });
});

test.describe('DemoGridFlex - Flex Column', () => {
  let browser: Browser;
  let page: Page;
  ...
  test('column flex grid renders correct data', async () => {
    const grid = page.locator('#F1\\.COL\\.G');
    const colHeaders = grid.locator('.grid-col-header');
    await expect(colHeaders).toHaveCount(3);
...
test.describe('DemoGridFlex - Fixed Positioning', () => {
  let browser: Browser;
  let page: Page;
...
test.describe('DemoGridFlex - All Grids', () => {
  let browser: Browser;
  let page: Page;
