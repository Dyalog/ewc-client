...
test.describe('Grid selection — row / col / all + copy as TSV', () => {
  let browser: Browser;
  let page: Page;
  ...
  test.beforeAll(async () => {
    ...
    page = await navigateToDemo(result.page, 'GridTitles', '.grid-table', 10000);
  });
  ...
  test('clicking a row header selects the whole row', async () => {
    await page.locator(`${grid} .grid-row-header`).nth(0).click();
    await new Promise((r) => setTimeout(r, 100));
    // All cells in row 1 get .range-selected.
    const rowCells = page.locator(`${grid} .grid-cell[data-row="1"].range-selected`);
    await expect(rowCells).toHaveCount(2);
  });

  test('clicking a column header selects the whole column', async () => {
    await page.locator(`${grid} .grid-col-header`).nth(0).click();
    await new Promise((r) => setTimeout(r, 100));
    const colCells = page.locator(`${grid} .grid-cell[data-col="1"].range-selected`);
    await expect(colCells).toHaveCount(3);
  });

  test('clicking the corner cell selects all data cells', async () => {
    await page.locator(`${grid} .grid-corner-cell`).click();
    await new Promise((r) => setTimeout(r, 100));
    const allSelected = page.locator(`${grid} .grid-cell.range-selected`);
  ...
  test('clicking a data cell clears the range — back to single-cell mode', async () => {
    ...
    await new Promise((r) => setTimeout(r, 100));
    const ranged = page.locator(`${grid} .grid-cell.range-selected`);
  ...
  test('Ctrl+C on a single-cell selection copies just that cell', async () => {
    await page.locator(`${grid} .grid-cell[data-row="2"][data-col="1"]`).click();
    await new Promise((r) => setTimeout(r, 100));
    await pressCopy(page);
    await new Promise((r) => setTimeout(r, 100));
    const text = await readClipboard(page);
  ...
  test('Shift+click extends the selection from the anchor cell', async () => {
    ...
    await new Promise((r) => setTimeout(r, 100));
    await page.locator(`${grid} .grid-cell[data-row="3"][data-col="2"]`).click({ modifiers: ['Shift'] });
    await new Promise((r) => setTimeout(r, 100));
    // All 6 cells should be in the range.
    await expect(page.locator(`${grid} .grid-cell.range-selected`)).toHaveCount(6);
  });

  test('Shift+click on a row header extends the row band', async () => {
    await page.locator(`${grid} .grid-row-header`).nth(0).click();
    await new Promise((r) => setTimeout(r, 100));
    await page.locator(`${grid} .grid-row-header`).nth(2).click({ modifiers: ['Shift'] });
    await new Promise((r) => setTimeout(r, 100));
  ...
  test('Shift+ArrowDown extends the selection downward', async () => {
    ...
    await expect(page.locator(`${grid} .grid-cell[data-col="1"].range-selected`)).toHaveCount(2);
  });
  ...
  test('Shift+ArrowRight then Shift+ArrowDown selects a rectangle', async () => {
    ...
    await expect(page.locator(`${grid} .grid-cell.range-selected`)).toHaveCount(4);
  });
  ...
  test('Plain arrow key after a range clears the selection', async () => {
    ...
    await page.locator(`${grid} .grid-row-header`).nth(0).click();
    await new Promise((r) => setTimeout(r, 100));
    await expect(page.locator(`${grid} .grid-cell.range-selected`)).toHaveCount(2);
    // Plain arrow → clear.
    await page.locator(grid).focus();
    await page.keyboard.press('ArrowDown');
    await new Promise((r) => setTimeout(r, 100));
    await expect(page.locator(`${grid} .grid-cell.range-selected`)).toHaveCount(0);
  });

  test('Mouse drag from one cell to another selects the rectangle', async () => {
    const start = page.locator(`${grid} .grid-cell[data-row="1"][data-col="1"]`);
    const end = page.locator(`${grid} .grid-cell[data-row="3"][data-col="2"]`);
...
// GridSelect event end-to-end: Grid client fires it, EWC server forwards
// to APL callback (CBGrid), which writes to F1.Log. We navigate to
// DemoGrid where the handler is registered and verify the log content.
test.describe('Grid GridSelect event — round-trip to APL handler', () => {
  let browser: Browser;
  let page: Page;
  ...
  test('clicking a row header logs "GridSelect: Row N" via CBGrid', async () => {
    // DemoGrid grid is F1.G; row headers are 1-indexed in the title column.
    await page.locator('#F1\\.G .grid-row-header').nth(1).click();
  ...
  test('clicking a column header logs "GridSelect: Col N"', async () => {
    await page.locator('#F1\\.G .grid-col-header').nth(2).click();
    await new Promise((r) => setTimeout(r, 400));
    const log = await page.locator('#F1\\.Log').textContent();
  ...
  test('clicking the corner cell logs "GridSelect: Range …"', async () => {
    await page.locator('#F1\\.G .grid-corner-cell').click();
    await new Promise((r) => setTimeout(r, 400));
    const log = await page.locator('#F1\\.Log').textContent();
  ...
  test('clicking a cell after a range logs "GridSelect: Cell [r,c]"', async () => {
    ...
    await page.locator('#F1\\.G .grid-row-header').nth(0).click();
    await new Promise((r) => setTimeout(r, 200));
    // Then click a cell to cancel.
    await page.locator('#F1\\.G .grid-cell[data-row="2"][data-col="2"]').click();
    await new Promise((r) => setTimeout(r, 400));
    const log = await page.locator('#F1\\.Log').textContent();
  ...
  // DemoGrid col 4 is "In Stock" (checkbox), col 5 is "Color" (Combo).
  ...
  test('drag ending over a Combo cell does NOT open the combo dropdown', async () => {
    ...
    const start = page.locator('#F1\\.G .grid-row-header').nth(0); // row header is safe to mousedown
    const end = page.locator('#F1\\.G .grid-cell[data-row="3"][data-col="4"]');
    ...
    // (DemoGrid's combos use native select; we check for an open dropdown
  ...
  test('drag-select ends on release even outside the grid', async () => {
    ...
    await page.locator('#F1\\.G .grid-row-header').nth(4).click();
    await new Promise((r) => setTimeout(r, 200));
    // Row 5's range — should be just row 5's cells.
    const newRow = page.locator('#F1\\.G .grid-cell[data-row="5"].range-selected');
