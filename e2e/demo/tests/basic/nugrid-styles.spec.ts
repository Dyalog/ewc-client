...
test.describe('DemoNuGridStyles - Selection visible over coloured cells', () => {
  let browser: Browser;
  let page: Page;
  ...
  async function selectBlock() {
    ...
    await page.locator(`${grid} .nugrid-cell[data-row="3"][data-col="3"]`).click({ modifiers: ['Shift'] });
    await new Promise(r => setTimeout(r, 150));
  }

  test('a range cell keeps its own BCol fill — selection does not overwrite the colour', async () => {
    await selectBlock();
    const cell = page.locator(`${grid} .nugrid-cell[data-row="2"][data-col="2"]`);
  ...
  test('a single focused cell highlights its right + bottom borders over a coloured fill', async () => {
    ...
    await new Promise(r => setTimeout(r, 200));
    const active = page.locator(`${grid} .nugrid-cell[data-row="2"][data-col="2"]`);
  ...
  test('a multi-cell selection draws a perimeter border via edge classes', async () => {
    await selectBlock();
    const corner = page.locator(`${grid} .nugrid-cell[data-row="1"][data-col="1"]`);
    ...
    await expect(page.locator(`${grid} .nugrid-cell[data-row="2"][data-col="2"]`))
