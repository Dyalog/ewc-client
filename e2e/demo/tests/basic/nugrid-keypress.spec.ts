...
//
// The DemoNuGridKeyPress demo registers KeyPress on BOTH the grid (F1.G) and the
// embedded text Edit template (F1.G.E1), and CBNuGridKey logs one line per fire
...
//     originates inside an embedded editor (src/components/NuGrid/index.jsx, the
...
test.describe('DemoNuGridKeyPress - embedded Edit KeyPress fires once (finding #14)', () => {
  let browser: Browser;
  let page: Page;
  ...
  test.beforeAll(async () => {
    ...
    page = await navigateToDemo(result.page, 'NuGridKeyPress', '.nugrid-table', 10000);
  });

  // Lines of the F1.Log Caption that record a KeyPress fire. CBNuGridKey prepends
  ...
  test('typing one char in an Edit cell logs exactly ONE KeyPress, from the Edit (not the grid)', async () => {
    ...
    // is what makes NuGrid treat it as `inEditor` and suppress its own KeyPress).
    ...
    // ...and it is the Edit's (newest is first because CBNuGridKey prepends).
