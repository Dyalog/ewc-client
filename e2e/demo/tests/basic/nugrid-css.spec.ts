...
test.describe('DemoNuGridCSS - CSS Property', () => {
  let browser: Browser;
  let page: Page;
  ...
  test.beforeAll(async () => {
    ...
    page = await navigateToDemo(result.page, 'NuGridCSS', '.nugrid-table', 10000);
  });
  ...
  test('styled grid has border-radius applied', async () => {
    const styledGrid = page.locator('.nugrid').first();
  ...
  test('unstyled grid does NOT have border-radius', async () => {
    const unstyledGrid = page.locator('.nugrid').nth(1);
  ...
  test('styled grid still renders column headers', async () => {
    const styledGrid = page.locator('.nugrid').first();
    const colHeaders = styledGrid.locator('.nugrid-col-header');
  ...
  test('styled grid still renders data cells', async () => {
    const styledGrid = page.locator('.nugrid').first();
    const cells = styledGrid.locator('.nugrid-cell');
  ...
  test('unstyled grid also renders correctly', async () => {
    const unstyledGrid = page.locator('.nugrid').nth(1);
    const colHeaders = unstyledGrid.locator('.nugrid-col-header');
    await expect(colHeaders).toHaveCount(3);

    const cells = unstyledGrid.locator('.nugrid-cell');
    await expect(cells).toHaveCount(9);
  });
});
